...
public class FortuneCategoryFactory {
    ...
    public FortuneCategoryModel create() {
        return new FortuneCategoryModel(fortuneCategoryRepo);
    }

    public FortuneCategoryModel loadById(Long categoryId) {
        FortuneCategoryEntity fortuneCategoryEntity = fortuneCategoryRepo.getById(categoryId);
        if (Objects.isNull(fortuneCategoryEntity)) {
            throw new ApiException(ErrorCode.Business.COMMON_OBJECT_NOT_FOUND, categoryId, "分类");
        }
        return new FortuneCategoryModel(fortuneCategoryEntity, fortuneCategoryRepo);
    }

    public List<FortuneCategoryModel> loadByIds(List<Long> categoryIds) {
        List<FortuneCategoryEntity> entities = fortuneCategoryRepo.getByIds(categoryIds);
        ...
        return entities.stream().map(item -> new FortuneCategoryModel(item, fortuneCategoryRepo)).toList();
    }
}
