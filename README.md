...
[在线体验](https://demo.fortuneboot.com/) | [官方网站](https://www.fortuneboot.com/) | [前端源码](https://github.com/shuaichi/FortuneBoot-Ui) | [提交 Issue](https://github.com/shuaichi/FortuneBoot/issues)
...
**FortuneBoot** 是一套完全开源的家庭与小微商户记账管理系统。我们致力于提供精简可靠、代码规范的财务数据中心。
不仅适合作为个人日常的财务管家，其纯粹的 **DDD（领域驱动设计）** 架构也非常适合作为 Java 开发者学习新一代微服务架构的极佳案例。

### ✨ 核心硬核特性
- 🏗️ **纯粹的 DDD 架构实践**：划分 Domain / Command / Query，彻底告别贫血模型，业务逻辑高度内聚。
- 🚀 **GraalVM Native Image 支持**：可编译为原生机器码，实现 **毫秒级启动** 与 **极低内存占用**。
- 🪶 **去除 Redis 强依赖 (v1.5.0)**：登录令牌持久化至数据库，部署架构极致轻量化。
- 🔄 **Flyway 无感数据库迁移**：告别手动导 SQL！启动即自动建表与刷入初始数据。
- 🔀 **MySQL / SQLite 双引擎**：支持 MySQL 生产级部署，同时内置 SQLite 模式，实现**真正的零配置开箱即用**。
...
| 功能模块 | 说明 |
|---|---|
| **多维账本体系** | 支持多账本隔离；支持高达 3 级的树形分类、交易对象管理与标签交叉统计。 |
| **单据闭环管理** | 专为职场人设计，完美串联“垫付 -> 创建报销/借款单 -> 挂靠账单 -> 销账”全流程。 |
| **周期记账 (Cron)** | 基于 Quartz + CRON 表达式，实现房贷、工资等固定收支的自动入账与错漏补偿。 |
| **归物 (资产成本化)** | 记录物品生命周期，自动计算日均/次均使用成本，帮你认清真实消费损耗。 |
| **安全回收站** | 账户、账本、分类、标签删除前均进入回收站，支持一键恢复，防止数据断裂。 |
| **多维度统计报表** | 资产负债概览、趋势折线图、收支分类/标签饼图分析。 |

### ⚙️ 系统基础支撑
| 功能模块 | 说明 |
|---|---|
| **权限与安全** | 精确到按钮级别的 RBAC 权限；支持数据范围隔离（全部/仅本人）。 |
| **多租户/分组** | 用户邀请机制，支持家庭成员、合伙人共同记账，数据绝对隔离。 |
| **日志与审计** | 详细记录登录行为、操作日志，支持接口响应耗时与异常堆栈追踪。 |
| **系统监控** | 查看服务器 CPU、内存、磁盘、JVM 运行状态，支持强制踢人下线。 |
...
### 方式一：SQLite 零配置启动（推荐初体验）
无需安装任何数据库或 Redis，直接克隆代码即可运行！
```bash
git clone https://github.com/shuaichi/FortuneBoot-Server.git
cd FortuneBoot-Server
mvn clean install -DskipTests
```
确保 `fortuneboot-starter/src/main/resources/application.yml` 中配置为：
```yaml
db:
  type: sqlite
  sqlite:
...
```
运行 `ApplicationStarter` 类，Flyway 会自动创建 `fortuneboot.db` 并初始化数据！
 ...
 **启动成功标志**
   ...
   ```
### 方式二：MySQL 生产级启动
1. 在 MySQL 中新建数据库 `fortune_boot`。
2. **配置数据库**

    修改 `application-dev.yml` (或 `prod`) 中的数据库连接：
   ```yaml
   spring:
     datasource:
       druid:
         url: jdbc:mysql://localhost:3306/fortune_boot?...
         username: root
...
以下配置支持通过**环境变量**直接覆盖，极大地提升了容器化部署体验：
...
| `SWAGGER_ENABLE`  | Swagger UI 开关（生产环境建议关闭） |
...
- **MySQL 模式**：迁移脚本位于 `fortuneboot-infrastructure/src/main/resources/db/migration/mysql/`，包含从 V1.0.0 到
  V1.5.0 的完整增量迁移链
...
| 版本 | 发布说明 |
|---|---|
| **v1.5.0** | 🚀 **重大重构**：去除 Redis 依赖，登录令牌持久化到数据库；优化 Native Image 构建配置。 |
| **v1.4.0** | 角色和用户新增超级管理员标识字段。 |
| **v1.3.0** | 新增单据管理功能（报销单/借款单），账单支持灵活挂靠。 |
| **v1.2.0** | 新增周期记账功能（Quartz 任务调度 + CRON 表达式），支持断点补偿。 |
| **v1.1.0** | 独创归物（GoodsKeeper）功能，智能计算物品持有与折旧成本。 |
| **v1.0.0** | 基础版本发布，涵盖多账本、多租户、RBAC、基础账单流水记录等核心业务。 |

---

## 💡 开发者注意事项

- **编码防坑**：IDEA 会自动将 `.properties` 文件编码设置为 ISO-8859-1，请在 **Settings > Editor > File Encodings > Properties Files**
  中设置为 UTF-8
- **代码规范**：请导入统一代码格式化模板：**Settings > Editor > Code Style > Java > Import Schema**，选择项目根目录下的
  `GoogleStyle.xml`
- **API 调试**：Swagger API 文档地址：`http://localhost:8080/v3/api-docs`，Swagger UI 地址：`http://localhost:8080/swagger-ui/index.html`
- **链路追踪**：项目中默认开启了 `TraceIdFilter`，请求的 TraceID 会自动注入 MDC，方便通过日志排查问题。
- **生产建议**：请务必修改 `token.secret`、`rsaPrivateKey`、`rsaPublicKey` 等安全配置，建议关闭 Swagger UI（设置环境变量 `SWAGGER_ENABLE=false`）

---

## 🤝 社区与交流

有任何问题或者建议，欢迎在 _Issues_ 中提出。你的每一次 PR 或 Issue 都是对开源最大的支持！

* **官方 QQ 交流群**：[![加入QQ群](https://img.shields.io/badge/1009576058-blue.svg)](https://qm.qq.com/q/M2zyt7vxyG)
* 如果觉得项目对您有帮助，请点亮右上角的 **Star ⭐️** 支持我们！
